...
def decodeEvent(featureVal):
	...
	params, rest = params.split(")")
	params = params.split(",")
	return retType, name, value, params
			...
			if line:
				if line[0] == "#":
					...
					if featureType in ["fun", "get", "set"]:
						...
						params = [ decodeParam(p) for p in params ]
						self.features[name] = {
							"FeatureType": featureType,
							"ReturnType": retType,
							"Value": value,
						...
						self.features[name] = {
							...
							"Value": value }
						self.order.append(name)
					elif featureType == "lex":
						name = featureVal.split('=')[0]
						values = featureVal.split('=')[1].split()
						lexid = values[0]
						prefixes = values[1:]
						self.features[name] = {
							"FeatureType": featureType,
						...
						self.order.append(name)
						currentComment = []
					elif featureType == "enu":
						name, value = featureVal.split("=", 1)
						prefixes = value.split()
						if name in self.features:
							self.features[name]["Prefixes"].extend(prefixes)
						else:
							self.features[name] = {
								"FeatureType": featureType,
								"Category": currentCategory,
